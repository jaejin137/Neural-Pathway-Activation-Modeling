...
class NRTL():
    ...
    Methods:     
        ...
        inital_AP_site - identify the node where from an action potiential originated
    ...
    def initialize_simulation(self,dt,delay,freq,nTest_pulses,AP_delay,stim_pulse,FEM_in_V):
        ...
        >>> cell_obj.initialize_simulation(dt,delay,freq,nTest_pulses,AP_delay,stim_pulse,FEM_in_V)
        ...
        self.nReq_APs = int(self.nIdeal_APs*0.8)
        ...
        stimulus_train_time = list(ii*h.dt for ii in range(0,len(stimulus_train))) # simulation time points
    ...
    def insert_vext(self,stim_amplitude): 
        ...
        for sec in self.allseclist:
            ...
            if len(self.FEM_in_V.shape) == 1:
                
                # scale pulse by stim_amplitude and convert FEM result from V to mV
                scaled_stim_pulse = self.stim_pulse * stim_amplitude  * self.FEM_in_V[iCompartment] * 1000

            # if the array of voltages is 2D then perform Fourier FEM
            elif len(self.FEM_in_V.shape) == 2:
                
                # scale pulse by stim_amplitude
                scaled_raw_pulse = self.stim_pulse * stim_amplitude
                
                # Fourier FEM
                n = 1024;                            # 1024 point DFT
                L = len(self.stim_pulse)             # Length of signal
                Y = np.fft.fft(scaled_raw_pulse,n);   # perform fft
                
                # reflect FEM results
                F1 = np.concatenate([self.FEM_in_V[:,iCompartment], np.conj(self.FEM_in_V[np.arange((L/2)-1,0,-1),iCompartment])]) 
                ...
                F5 = sig.resample(F4,1/h.dt)  # resample to match h.dt
                scaled_stim_pulse = F5 * 1000 # convert result from V to mV
                                
            else:
                raise Exception("Voltage value array has more than two dimentions")   
            
            # construct a stimulus pulse train from a scaled_stim_pulse
            stimulus_train = self.pulse_train(scaled_stim_pulse)
                
            v_ext.append(h.Vector(stimulus_train))
    ...
    #---------------------------------------------------------------------------
    def inital_AP_site(self):
        """
        Find AP initiation site.
        
        >>> cell_obj.inital_AP_site()
...
class STN(NRTL):
    ...
    Attributes:
    v_init:          (double) cell inital voltage
    ...
    def construct_geom(self,STNstrip,fiberD=2):        
        ...
        for ii in range(0,self.nNodes-1):
            self.node[ii].nseg = 1
    ...
    def insert_APcounters(self):
        self.recNode = 27#######################DEBUG: we use node 11 (in python is 10) here because of debugging only. This value *should* be 27, which means we are recording from node 28, which is the furthest away from soma
        ...
        for ii in range(self.nNodes):
            ...
            self.membrane_v.append(h.Vector())
            self.membrane_v[ii].record(self.node[ii](0.5)._ref_v)
        
        # for jj in range(self.nSoma):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.soma[jj](0.5)._ref_v)
        # for jj in range(self.nMysa):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.MYSA[jj](0.5)._ref_v)
        # for jj in range(self.nFlut):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.FLUT[jj](0.5)._ref_v)
...
class PPNType2(NRTL):
    ...
    def construct_geom(self,cell_data,fiberD=2):        
        ...
        space_p2 = 0.004
        space_i = 0.004
        if fiberD==1: axonD=0.8; nodeD=0.7; paraD1=0.7; paraD2=0.8; deltax=150; nl=20
...
class Axon(NRTL):
    ...
    def insert_APcounters(self):
        ...
        #record from all mysa
        # for jj in range(self.nMysa):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.MYSA[jj](0.5)._ref_v)
            
        # # record from all flut
        # for jj in range(self.nFlut):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.FLUT[jj](0.5)._ref_v)
            
        # # record from all stin
        # for jj in range(self.nStin):
            # self.membrane_v.append(h.Vector())
            # self.membrane_v[jj].record(self.STIN[jj](0.5)._ref_v)

        self.membrane_v_time = h.Vector()
        self.membrane_v_time.record(h._ref_t)
class GPi(NRTL):
    """
    This GPi class inherits the NRTL metaclass methods.
    ...
    def construct_geom(self,GPi,fiberD=2):        
        """
        Build a multicompartment cell, generate a section list, and insert AP 
        counters 
        
        >>> axon_obj.construct_geom(fiberD,GPi)
        
        Inputs:
            fiberD:    (double) fiber diameter (um): must be 1 or 2
            GPi: (structure) imported from a .mat and contains all the compartment information
        """
        # Set axon parameters and build axons
        h.celsius = 36  # temperature in Celsius
        self.v_init = -60 
        ...
        my_gk_KDRs = .001
        my_gna_Na = .02
        my_gna_NaL = 2.8e-5 *0.01 # edited 4/6/2017 to decrease spontaneous spiking
        my_gna_Na = .02 *0.5 # edited 4/6/2017 to decrease spontaneous spiking, *0.55 is the spontaneous firing threshold.
        my_gk_Ih = .0002
        ...
        for ii in range(0,int(self.nSoma)):
            ...
            self.soma[ii].insert('sKCa')
            self.soma[ii].insert('KDRf') # Model for K+ Delayed Rectifier Current (fast-deactivating)
            self.soma[ii].insert('KDRs') # Model for K+ Delayed Rectifier Current (slow-deactivating)
            self.soma[ii].insert('Na') # Model for NA+ Current
            self.soma[ii].insert('NaL') # Model for NA+ Leak Current  
            self.soma[ii].insert('Ih') # 
            self.soma[ii].insert('extracellular')   
        ...
        # create all nodes that are not the last node and insert mechanisms
        for ii in range(0,self.nNodes-1):
            self.node[ii].nseg = 1
            self.node[ii].Ra = rhoa/10000
            self.node[ii].cm = 2
        ...
        # Somas
        for ii in range(0,self.nSoma-1):
            self.soma[ii+1].connect(self.soma[ii],(1),(0))
            
        self.dend[0].connect(self.soma[7],(1),(0))
        
        #Dendrites
        for ii in range(0,14):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
        
        self.dend[15].connect(self.dend[6],(1),(0))
        self.dend[16].connect(self.dend[15],(1),(0))
        self.dend[17].connect(self.dend[0],(1),(0))
        
        for ii in range(17,24):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[25].connect(self.dend[0],(1),(0))    
        
        for ii in range(25,37):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[38].connect(self.dend[28],(1),(0))   
        self.dend[39].connect(self.dend[38],(1),(0))
        self.dend[40].connect(self.dend[39],(1),(0))
        self.dend[41].connect(self.soma[0],(1),(0))

        for ii in range(41,51):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[52].connect(self.dend[41],(1),(0))   
        
        for ii in range(52,59):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.initseg[0].connect(self.soma[1],(1),(0)) 
        self.node[0].connect(self.initseg[0],(1),(0)) 
    ...
    def insert_APcounters(self):
        ...
        # # record from initseg
        # self.membrane_v.append(h.Vector())
        # self.membrane_v[jj+1].record(self.initseg[0](0.5)._ref_v)
...
class GPetype2(NRTL):
    """
    This GPe type 2 class inherits the NRTL metaclass methods.
    ...
    def construct_geom(self,GPe2,fiberD=2):        
        """
        Build a multicompartment cell, generate a section list, and insert AP 
        counters 
        
        >>> axon_obj.construct_geom(fiberD,GPe2)
        
        Inputs:
            fiberD:    (double) fiber diameter (um): must be 1 or 2
            GPe2: (structure) imported from a .mat and contains all the compartment information
        """
        # Set axon parameters and build axons
        h.celsius = 36  # temperature in Celsius
        self.v_init = -60 
        ...
        soma = GPe2.soma
        dend = GPe2.dend
        initseg = GPe2.initseg
        nodes = GPe2.node
        mysa = GPe2.mysa
        flut = GPe2.flut
        stin = GPe2.stin
        #self.nStinPerStretch = GPe2.nStinPerStretch # this was in laura's PPN code, but we don't need it for STN
        ...
        my_gk_KDRf = .004  
        my_gk_KDRs = .001
        my_gna_Na = .02*0.5 # edited 3/23/2017 to decrease spontaneous spiking, *0.55 is the spontaneous firing threshold.
        my_gna_NaL = 2.0e-5 *0.01 # edited 3/23/2017 to decrease spontaneous spiking  
        my_gk_Ih = .0002
        ...
        # create last node (passive) and insert mechanisms  
        self.node[9].nseg = 1 # number of segments
        self.node[9].Ra = rhoa/10000 # axial resistivity (Ohm-cm)
        self.node[9].cm = 2
        self.node[9].insert('pas') # insert passive current
        self.node[9].insert('extracellular') # insert extracellular mechanism
        for seg in self.node[9]:
            seg.pas.g = 0.0001 # S/cm^2
            seg.pas.e = self.v_init 
            seg.xraxial[0] = Rpn0 # MOhms/cm
            seg.xg[0] = 1e10
            seg.xc[0] = 0

        for ii in range(10,22):
            self.node[ii].nseg = 1
            self.node[ii].Ra = rhoa/10000
            self.node[ii].cm = 2
            self.node[ii].insert('axnode70') 
        ...
        # create last node (passive) and insert mechanisms  
        self.node[22].nseg = 1 # number of segments
        self.node[22].Ra = rhoa/10000 # axial resistivity (Ohm-cm)
        self.node[22].cm = 2
        self.node[22].insert('pas') # insert passive current
        self.node[22].insert('extracellular') # insert extracellular mechanism
        for seg in self.node[22]:
            seg.pas.g = 0.0001 # S/cm^2
            seg.pas.e = self.v_init 
            seg.xraxial[0] = Rpn0 # MOhms/cm
            seg.xg[0] = 1e10
        ...
        # create last node (passive) and insert mechanisms  
        self.node[28].nseg = 1 # number of segments
        self.node[28].Ra = rhoa/10000 # axial resistivity (Ohm-cm)
        self.node[28].cm = 2
        self.node[28].insert('pas') # insert passive current
        self.node[28].insert('extracellular') # insert extracellular mechanism
        for seg in self.node[28]:
            seg.pas.g = 0.0001 # S/cm^2
            seg.pas.e = self.v_init 
            seg.xraxial[0] = Rpn0 # MOhms/cm
            seg.xg[0] = 1e10
        ...
        self.node[37].nseg = 1 # number of segments
        self.node[37].Ra = rhoa/10000 # axial resistivity (Ohm-cm)
        self.node[37].cm = 2
        self.node[37].insert('pas') # insert passive current
        self.node[37].insert('extracellular') # insert extracellular mechanism
        for seg in self.node[37]:
            seg.pas.g = 0.0001 # S/cm^2
            seg.pas.e = self.v_init 
            seg.xraxial[0] = Rpn0 # MOhms/cm
            seg.xg[0] = 1e10
        ...
        self.dend[0].connect(self.soma[3],(1),(0))
        
        #Dendrites
        for ii in range(0,7):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
        
        self.dend[8].connect(self.soma[8],(1),(0))
        
        for ii in range(8,78):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[79].connect(self.dend[14],(1),(0))    
        
        for ii in range(79,102):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[103].connect(self.dend[9],(1),(0))   
        
        for ii in range(103,115):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[116].connect(self.dend[107],(1),(0))   
        
        for ii in range(116,136):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[137].connect(self.dend[129],(1),(0))   
        
        for ii in range(137,142):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[143].connect(self.soma[0],(1),(0))  
        
        for ii in range(143,159):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[160].connect(self.dend[145],(1),(0))   
            
        for ii in range(160,205):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[206].connect(self.dend[202],(1),(0))   
        
        for ii in range(206,207):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[208].connect(self.dend[144],(1),(0))   
        
        for ii in range(208,229):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
                      
        self.dend[230].connect(self.dend[209],(1),(0))
         
        for ii in range(230,251):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[252].connect(self.dend[241],(1),(0))   
        
        for ii in range(252,257):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[258].connect(self.dend[241],(1),(0))   
         
        for ii in range(258,266):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.dend[267].connect(self.dend[208],(1),(0))   
        
        for ii in range(267,274):
            self.dend[ii+1].connect(self.dend[ii],(1),(0))
            
        self.initseg[0].connect(self.soma[9],(1),(0))
        self.node[0].connect(self.initseg[0],(1),(0)) 
        ...
        self.MYSA[8].connect(self.node[4],(1),(0))
        self.FLUT[8].connect(self.MYSA[8],(1),(0))
        self.FLUT[9].connect(self.FLUT[8],(1),(0))

        for ii in range(5,9): 
            self.MYSA[2*ii-1].connect(self.FLUT[2*ii-1],(1),(0))
            self.node[ii].connect(self.MYSA[2*ii-1],(1),(0))
            self.MYSA[2*ii].connect(self.node[ii],(1),(0))
            self.FLUT[2*ii].connect(self.MYSA[2*ii],(1),(0))
            self.STIN[3*ii-3].connect(self.FLUT[2*ii],(1),(0))
            self.STIN[3*ii-2].connect(self.STIN[3*ii-3],(1),(0))
            self.STIN[3*ii-1].connect(self.STIN[3*ii-2],(1),(0))
            self.FLUT[2*ii+1].connect(self.STIN[3*ii-1],(1),(0))

        self.MYSA[17].connect(self.FLUT[17],(1),(0))
        self.node[9].connect(self.MYSA[17],(1),(0))
        self.STIN[24].connect(self.FLUT[8],(1),(0))

        for ii in range(9,18): 
            self.STIN[3*ii-2].connect(self.STIN[3*ii-3],(1),(0))
            self.STIN[3*ii-1].connect(self.STIN[3*ii-2],(1),(0))
            self.FLUT[2*ii].connect(self.STIN[3*ii-1],(1),(0))
            self.MYSA[2*ii].connect(self.FLUT[2*ii],(1),(0))
            self.node[ii+1].connect(self.MYSA[2*ii],(1),(0))
            self.MYSA[2*ii+1].connect(self.node[ii+1],(1),(0))
            self.FLUT[2*ii+1].connect(self.MYSA[2*ii+1],(1),(0))
            self.STIN[3*ii].connect(self.FLUT[2*ii+1],(1),(0))


        for ii in range(18,21): 
            self.STIN[3*ii-2].connect(self.STIN[3*ii-3],(1),(0))
            self.FLUT[2*ii].connect(self.STIN[3*ii-2],(1),(0))
            self.MYSA[2*ii].connect(self.FLUT[2*ii],(1),(0))
            self.node[ii+1].connect(self.MYSA[2*ii],(1),(0))
            self.MYSA[2*ii+1].connect(self.node[ii+1],(1),(0))
            self.FLUT[2*ii+1].connect(self.MYSA[2*ii+1],(1),(0))
            self.STIN[3*ii-1].connect(self.FLUT[2*ii+1],(1),(0))
            self.STIN[3*ii].connect(self.STIN[3*ii-1],(1),(0))

        self.STIN[61].connect(self.STIN[60],(1),(0))
        self.FLUT[42].connect(self.STIN[61],(1),(0))
        self.MYSA[42].connect(self.FLUT[42],(1),(0))
        self.node[22].connect(self.MYSA[42],(1),(0))
        self.STIN[62].connect(self.STIN[51],(1),(0))

        for ii in range(22,24): 
            self.STIN[3*ii-3].connect(self.STIN[3*ii-4],(1),(0))
            self.FLUT[2*ii-1].connect(self.STIN[3*ii-3],(1),(0))
            self.MYSA[2*ii-1].connect(self.FLUT[2*ii-1],(1),(0))
            self.node[ii+1].connect(self.MYSA[2*ii-1],(1),(0))
            self.MYSA[2*ii].connect(self.node[ii+1],(1),(0))
            self.FLUT[2*ii].connect(self.MYSA[2*ii],(1),(0))
            self.STIN[3*ii-2].connect(self.FLUT[2*ii],(1),(0))
            self.STIN[3*ii-1].connect(self.STIN[3*ii-2],(1),(0))

        self.STIN[69].connect(self.STIN[68],(1),(0))

        for ii in range(24,27): 
            self.STIN[3*ii-2].connect(self.STIN[3*ii-3],(1),(0))
            self.FLUT[2*ii-1].connect(self.STIN[3*ii-2],(1),(0))
            self.MYSA[2*ii-1].connect(self.FLUT[2*ii-1],(1),(0))
            self.node[ii+1].connect(self.MYSA[2*ii-1],(1),(0))
            self.MYSA[2*ii].connect(self.node[ii+1],(1),(0))
            self.FLUT[2*ii].connect(self.MYSA[2*ii],(1),(0))
            self.STIN[3*ii-1].connect(self.FLUT[2*ii],(1),(0))
            self.STIN[3*ii].connect(self.STIN[3*ii-1],(1),(0))

        self.STIN[79].connect(self.STIN[78],(1),(0))
        self.FLUT[53].connect(self.STIN[79],(1),(0))
        self.MYSA[53].connect(self.FLUT[53],(1),(0))
        self.node[28].connect(self.MYSA[53],(1),(0))
        self.STIN[80].connect(self.STIN[68],(1),(0))

        for ii in range(28,36): 
            self.FLUT[2*ii-2].connect(self.STIN[3*ii-4],(1),(0))
            self.MYSA[2*ii-2].connect(self.FLUT[2*ii-2],(1),(0))
            self.node[ii+1].connect(self.MYSA[2*ii-2],(1),(0))
            self.MYSA[2*ii-1].connect(self.node[ii+1],(1),(0))
            self.FLUT[2*ii-1].connect(self.MYSA[2*ii-1],(1),(0))
            self.STIN[3*ii-3].connect(self.FLUT[2*ii-1],(1),(0))
            self.STIN[3*ii-2].connect(self.STIN[3*ii-3],(1),(0))
            self.STIN[3*ii-1].connect(self.STIN[3*ii-2],(1),(0))  

        self.FLUT[70].connect(self.STIN[104],(1),(0))
        self.MYSA[70].connect(self.FLUT[70],(1),(0))
        self.node[37].connect(self.MYSA[70],(1),(0))      
...
class GPetype3(NRTL):
    """
    This GPe type 3 class inherits the NRTL metaclass methods.
    ...
    def construct_geom(self,GPe1,fiberD=2):        
        """
        Build a multicompartment cell, generate a section list, and insert AP 
        counters 
        
        >>> axon_obj.construct_geom(fiberD,GPe1)
        
        Inputs:
            fiberD:    (double) fiber diameter (um): must be 1 or 2
            GPe1: (structure) imported from a .mat and contains all the compartment information
        """
        # Set axon parameters and build axons
        h.celsius = 36  # temperature in Celsius
        self.v_init = -60 
        ...
        soma = GPe1.soma
        dend = GPe1.dend
        initseg = GPe1.initseg
        node = GPe1.node
        mysa = GPe1.mysa
        flut = GPe1.flut
        stin = GPe1.stin
        #self.nStinPerStretch = GPe1.nStinPerStretch # this was in laura's PPN code, but we don't need it for STN
        ...
        self.initseg[0].connect(self.soma[9],(1),(0))
        self.initseg[1].connect(self.initseg[0],(1),(0))
        self.node[0].connect(self.initseg[0],(1),(0)) 
        ...
        for ii in range(self.nNodes):
            h.pt3dadd(node[ii,0],node[ii,1],node[ii,2],nodeD,sec=self.node[ii])
            h.pt3dadd(node[ii,4],node[ii,5],node[ii,6],nodeD,sec=self.node[ii])
        for ii in range(self.nMysa):
            h.pt3dadd(mysa[ii,0],mysa[ii,1],mysa[ii,2],paraD1,sec=self.MYSA[ii])
            h.pt3dadd(mysa[ii,4],mysa[ii,5],mysa[ii,6],paraD1,sec=self.MYSA[ii])
        for ii in range(self.nFlut):
