...
% Jaejin 02/2024, functions in BOX
% Use for STN-Motor Cortex pathways (STN, Motor Cortex, STN-Motor pathway)
...
nAxon = 64; %# of axons (GPeGPi 750, GPeSTN 1250, PutGPe 784, PutGPeGPi 816)
...
geom_name = 'STNMC'; %Name (abbreviation) of the pathway to map
...
[stl.STN.Points,stl.STN.ConnectivityList] = stlread1('R_STN.stl');
%[stl.GPiRing.Points,stl.GPiRing.ConnectivityList] = stlread1('R_GPiRing_new2.stl');
[stl.MC.Points,stl.MC.ConnectivityList] = stlread1('R_Motor.stl');
[stl.STNMCPath.Points,stl.STNMCPath.ConnectivityList] = stlread1('R_STN_to_Cortex_path.stl');
...
stl.MCTri = zeros(length(stl.MC.ConnectivityList),9);
%stl.GPiRingTri = zeros(length(stl.GPiRing.ConnectivityList),9);
%stl.PutTri = zeros(length(stl.Put.ConnectivityList),9);
stl.STNMCPathTri = zeros(length(stl.STNMCPath.ConnectivityList),9);
...
outFolder = 'C:\Users\jaejin\gdrive_jaejin\Work\PAM\Barb\Modeling_Data_rev1\Data_Processing\Axons\STNMC';
...
% Added by JL
if strcmp (geom_name, 'STNMC') ==1   %populate cells in STN if applicable
    final_pts = zeros(3,nSeedPoints,(nContour));
    for i = 1:length(stl.STNTri)
        stl.STNTri(i,:) = [stl.STN.Points(stl.STN.ConnectivityList(i,1),:),...
        stl.STN.Points(stl.STN.ConnectivityList(i,2),:),...
    ...
    for i = 1:length(stl.MCTri)
        stl.MCTri(i,:) = [stl.MC.Points(stl.MC.ConnectivityList(i,1),:),...
        stl.MC.Points(stl.MC.ConnectivityList(i,2),:),...
        stl.MC.Points(stl.MC.ConnectivityList(i,3),:)];
    end  
    for i = 1:length(stl.STNMCPathTri)
        stl.STNMCPathTri(i,:) = [stl.STNMCPath.Points(stl.STNMCPath.ConnectivityList(i,1),:),...
        stl.STNMCPath.Points(stl.STNMCPath.ConnectivityList(i,2),:),...
        stl.STNMCPath.Points(stl.STNMCPath.ConnectivityList(i,3),:)];
    end
end
%% ----------------------------------------------------------------------------------------------------------------------------
...
% Added by JL
if strcmp (geom_name, 'STNMC') ==1   %populate cells in GPe if applicable
    %[STN,STNMCPath,MC] = PopSTNMC(stl,nSeedPoints);
    [STN,MC] = PopSTNMC(stl,nSeedPoints);
end
...
% Added by JL
if strcmp (geom_name, 'STNMC') ==1  
    final_pts(:,:,1)=STN;
    %final_pts(:,:,2)=STNMCPath;
    final_pts(:,:,3)=MC;
end
%% ----------------------------------------------------------------------------------------------------------------------------
...
% Added by JL
if strcmp (geom_name, 'STNMC') ==1  
    [fiber_pts,final_connection_pts1] = axon_4_connect_curved_STNMC(final_pts,nAxon,nSeedPoints);
    axonpop = axon_4_axonpop_struct_fiberD(fiber_pts, axon_dia); 
end
%% ----------------------------------------------------------------------------------------------------------------------------
...
if strcmp (geom_name, 'STNMC') ==1  
    h1=figure; 
    g = trisurf(stl.STN.ConnectivityList, stl.STN.Points(:,1), stl.STN.Points(:,2), stl.STN.Points(:,3),'FaceColor','c','EdgeColor',grey);
    g.FaceAlpha = 1;
    hold on;
    
    g = trisurf(stl.MC.ConnectivityList, stl.MC.Points(:,1), stl.MC.Points(:,2), stl.MC.Points(:,3),'FaceColor','m','EdgeColor',grey);
    g.FaceAlpha = 1;
    hold on;

    g = trisurf(stl.STNMCPath.ConnectivityList, stl.STNMCPath.Points(:,1), stl.STNMCPath.Points(:,2), stl.STNMCPath.Points(:,3),'FaceColor','y','EdgeColor',grey);
    g.FaceAlpha = .3;
    hold on;
    ...
    cd(outFolder)
    savefig(h1,[subj '_' side '_STNMCAxon'])
end
...
if strcmp (geom_name, 'GPeSTN') ==1  
 ...
 % Added by JL
if strcmp (geom_name, 'STNMC') ==1  
    save([subj '_' side '_STNMC_Output_Workspace'])
    save([subj '_' side '_STNMC_Output_axonpop'], 'axonpop')
end
...
if strcmp (geom_name, 'STNMC') ==1
    % add to axonpop struct
    STNx=num2cell(cellbod_final(:,1));
        [axonpop(:).cellbod_STNx]= STNx{:};
    STNy=num2cell(cellbod_final(:,2));
        [axonpop(:).cellbod_STNy]= STNy{:};
    STNz=num2cell(cellbod_final(:,3));
        [axonpop(:).cellbod_STNz]= STNz{:};
end
...
outFolder = 'C:\Users\jaejin\gdrive_jaejin\Work\PAM\Barb\Modeling_Data_rev1\Data_Processing\Axons\STNMC\Cleaned\';
if ~exist(outFolder,'dir')
mkdir(outFolder)
end
cd(outFolder)
